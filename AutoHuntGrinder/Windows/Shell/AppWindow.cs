using AutoHuntGrinder.Windows.Pages;
...
namespace AutoHuntGrinder.Windows.Shell;
...
public sealed class AppWindow : Window, IDisposable
{
    ...
    public AppWindow(Plugin plugin) : base("Auto Hunt Grinder###AutoHuntGrinderMain", BaseFlags)
    {
        this.plugin = plugin;
        Size = DefaultSize;
    ...
    private void DrawBody(Vector2 windowPos, Vector2 windowSize, float headerHeight, float windowRounding)
    {
        ...
        using (var rail = ImRaii.Child("##ahg_rail", new Vector2(railWidth, bodyHeight), false, ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse))
        {
        ...
        using (var content = ImRaii.Child("##ahg_page", new Vector2(windowSize.X - railWidth - rightInset, bodyHeight), false, ImGuiWindowFlags.AlwaysUseWindowPadding))
        {
            if (content) DrawPage();
        }
