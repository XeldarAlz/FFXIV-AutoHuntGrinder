...
internal sealed partial class LogPage
{
    ...
    private const string ListId = "##ahg_log_list";
    private const string MenuId = "##ahg_log_menu";
    ...
    private void DrawJumpPill(float scrollY, float visibleHeight, float innerWidth)
    {
        ...
        if (PillButton.Draw("##ahg_log_jump", label, Styling.AccentGlow, PillButton.Emphasis.Filled, FontAwesomeIcon.ArrowDown, height: Layout.ActionPillHeight))
        {
            follow = true;
            newWhileDetached = 0;
        }
    ...
    private void DrawEmpty()
    {
        ...
        if (bufferedCount > 0)
        {
            ...
            if (PillButton.Draw("##ahg_log_reset_empty", Loc.T(L.Log.ResetFilters), Styling.AccentGlow, PillButton.Emphasis.Tinted, FontAwesomeIcon.Undo,
        ...
        var footer = Loc.T(L.Log.Footer, AhgConstants.LogPrefix);
