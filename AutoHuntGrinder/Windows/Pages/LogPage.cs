...
internal sealed partial class LogPage
{
    ...
    private static readonly string[] chipIds = ["##ahg_log_verbose", "##ahg_log_debug", "##ahg_log_info", "##ahg_log_warning", "##ahg_log_error"];
    ...
    private void DrawToolbar()
    {
        ...
        if (SearchField.Draw("##ahg_log_search", Loc.T(L.Log.SearchHint), ref search, ref searchFocused, searchWidth, Layout.ActionPillHeight, focusSearch))
        {
            ClearSelection();
        }
        ...
        if (PillButton.Draw("##ahg_log_copy", copyLabel, Styling.AccentGlow, PillButton.Emphasis.Filled, FontAwesomeIcon.Copy,
        ...
        if (PillButton.Draw("##ahg_log_clear", clearLabel, clearArmed ? Styling.AccentRose : Styling.TextSecondary,
    ...
    private void DrawFilters()
    {
        ...
        if (source is not null)
        {
            ...
            ImGui.SetCursorScreenPos(new Vector2(x, y));
            if (FilterChip.Draw("##ahg_log_source", label, null, Styling.AccentNebula, true, height, FontAwesomeIcon.Times, Loc.T(L.Log.SourceChipTooltip)))
            {
                source = null;
                ClearSelection();
            }
