...
internal sealed partial class LogPage
{
    ...
    private const string InspectorDetailId = "##ahg_log_detail";
    ...
    private void DrawSelectionBar()
    {
        ...
        if (PillButton.Draw("##ahg_log_sel_clear", clearLabel, Styling.TextSecondary, PillButton.Emphasis.Ghost, FontAwesomeIcon.Times,
        ...
        if (PillButton.Draw("##ahg_log_sel_copy", copyLabel, Styling.AccentGlow, PillButton.Emphasis.Tinted, FontAwesomeIcon.Copy,
                height: Layout.ActionPillHeight))
        {
            CopyRange(low, high);
        }
    ...
    private void DrawLineCard(int index, float height)
    {
        ...
        if (IconButton.Draw(FontAwesomeIcon.Times, "##ahg_log_inspect_close", buttonSize, Styling.TextDim, Loc.T(L.Log.Close)))
        {
            ClearSelection();
        }

        ImGui.SetCursorScreenPos(new Vector2(closeX - buttonSize - HeaderGap * scale * 0.5f, headerMidY - buttonSize * 0.5f));
        if (IconButton.Draw(FontAwesomeIcon.Copy, "##ahg_log_inspect_copy", buttonSize, Styling.AccentGlowSoft, Loc.T(L.Log.CopyLine)))
        {
    ...
    private float DrawShowing(Vector2 origin)
    {
        ...
        var hit = Hit.Area("##ahg_log_reset", resetSize);
